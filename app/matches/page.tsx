...
export default function Matches(){
 ...
 <section id="faculty"><h2>Faculty matches</h2><div className="grid">{fm.slice(0,6).map(({p,paper,signal,score,reason},i)=><article className="card" key={p.id}><span className="meta">{Math.round(score*100)}% topical fit</span><div className="strength"><i style={{width:`${score*100}%`}}/></div><h3>{p.name}</h3><p>{p.title}, {p.departments[0]}</p><p><strong>Why this match:</strong> {reason}</p><div className="chips"><span className="chip">{signal.recent_output} papers since {idx.meta.recent_cutoff}</span><span className="chip">latest publication {signal.publication_recency}</span>{opted&&i===0&&<span className="chip">explicitly open to students</span>}</div><a href={p.profile_url}>View source profile</a></article>)}</div></section>
